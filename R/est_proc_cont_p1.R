est_proc_cont_p1<-function(x,y,g_suit,c,c_inherit,start,mc.cores,PSOCK,parallel_trace,dt,twosample_data=NULL,nlminb_control){
  #twosample_data should be updated according to suit.
  mypaste<-function(m){
  ...
  my_task<-function(my_loc){
    #g_suit,twosample_data,start,c_inherit,c,x,y,nlminb_control,parallel_trace,mc.cores
    t0<-Sys.time()
    my_proc<-0
    pid<-my_loc[[2]]
    my_loc<-my_loc[[1]]
    ...
    withCallingHandlers(
      {
        ...
        for(i in 1:n1){
          ...
          se<-cbind(se,sqrt(diag(vcov)))
          cor<-c(cor,vcov[1,2]/sqrt(vcov[1,1])/sqrt(vcov[2,2]))
          eff<-cbind(eff,fit1$estimate[1:2])
        ...
        colnames(eff)<-colnames(se)<-colnames(z)<-names(cor)<-colnames(g_suit)
      }
    ...
    return(list(eff,se,cor,z,w1,w2))
  }
  ...
  mybind_list_parallel<-function(list_parallel){
    ...
    for(i in 1:length(list_parallel)){
      ...
      out4<-cbind(out4,list_parallel[[i]][[4]])
  ...
  if(is.null(twosample_data)){
    ...
    return(list(eff=myfit[[1]],se=myfit[[2]],cor=myfit[[3]],b1_indiv=myfit[[4]]))
  }else{
    myfit<-mybind_list_parallel(myfit)
    return(list(eff=myfit[[1]],se=myfit[[2]],cor=myfit[[3]],b1_indiv=myfit[[4]],w1=myfit[[5]],w2=myfit[[6]]))
  }
}

est_proc_bi_p3_f<-function(beta,m_matrix,sigma1_matr,k_matrix,sign_k1,sign_k2,
                           p1_sp=NULL,p2_sp=NULL,r_sp=NULL,model_u2=T,individual=F,vcov_est=F,t_b1=F,beta_loc=NULL){
  if(!is.null(beta_loc)){
    beta0<-rep(NA,7)
    beta0[4]<-0
    beta0[beta_loc]<-beta
  ...
  o<-b1_t*k_matrix[,3]
  z1_log<-log((exp(k_matrix[,1])-1)*o+1)
  z2_log<-log((exp(k_matrix[,2])-1)*o+1)
  
  u21<-u2*sign_k1
  u22<-2*u2*sign_k2
  
  u1_m<-u1*k_matrix[,1]+u21+z1_log
  u2_m<-u1*k_matrix[,2]+u22+z2_log
